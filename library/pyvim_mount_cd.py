...
---
author: Arthur Reyes
module: pyvim_mount_cd
description: 
 - Mount an iso on Virtual Machine
version_added: "0.1"
requirements:
  - pyVim
notes:
...
options:
  ...
  media:
    description:
      - Full path to iso written as "[datastore] path/to/media/file
    required: true
'''
...
def main():
  ...
  if hasattr(cdrom.backing, 'fileName') and cdrom.backing.fileName == media:
  ...
  cdrom_spec.device.backing.fileName = media
  ...
  if str(dev_changes[0]).find(media) > 0:
    module.exit_json(changed=True)
  else:
    module.fail_json(msg='media not found or not mounted %s' % (media))
